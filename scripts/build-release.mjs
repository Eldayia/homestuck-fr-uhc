...
import { createDeterministicZip, sha256File } from "./zip-utils.mjs"
...
try {
  const first = await pack(join(workDirectory, "first", archiveName))
  const second = await pack(join(workDirectory, "second", archiveName))
  if (first.hash !== second.hash) throw new Error("Les deux archives ZIP MODE B ne sont pas reproductibles")
  ...
  process.stdout.write(`[OK] ZIP MODE B reproductible: ${archiveName} (${first.files.length} fichiers, ${first.hash})\n`)
} finally {
...
async function pack(destination) {
  ...
  await createDeterministicZip(destination, resolve("."), files)
  return { path: destination, files, hash: await sha256File(destination, createHash) }
}
...
function listPackageFiles() {
  ...
  if (!result || !Array.isArray(result.files)) throw new Error("Résultat npm pack --dry-run invalide")
  return result.files.map((entry) => entry.path).sort()
}

function assertAllowed(path) {
  const allowedExact = new Set([
    "CODE_OF_CONDUCT.md", "CONTRIBUTING.md", "CREDITS.md", "LICENSE", "NOTICE", "README.md", "THIRD_PARTY.md",
    "hsfr", "hsfr.cmd", "package.json",
  ])
